...
namespace HoloToolkit.Unity
{
    ...
    public class WorldAnchorManager : Singleton<WorldAnchorManager>
    {
        ...
        protected void DoAnchorOperation(AnchorAttachmentInfo anchorAttachmentInfo)
        {
            if (ShowDetailedLogs)
            {
                Debug.LogFormat("[WorldAnchorManager] Doing anchor operation with the anchorId: \"{0}\", anchoredGameObject: \"{1}\" and anchor operation: \"{2}\""
                    , anchorAttachmentInfo.AnchorName
                    , anchorAttachmentInfo.AnchoredGameObject
                    , anchorAttachmentInfo.Operation);
            }
            ...
            switch (anchorAttachmentInfo.Operation)
            {
                case AnchorOperation.Save:
                    ...
                    {
                        if (AnchorGameObjectReferenceList.ContainsKey(anchorId)) {
                            AnchorGameObjectReferenceList.TryGetValue(anchorId, out anchoredGameObject);
                            Debug.LogFormat("[WorldAnchorManager] Anchor to load already is in use by \"{0}\". Cancelling load.", anchoredGameObject.name);
                            return;
                        }
