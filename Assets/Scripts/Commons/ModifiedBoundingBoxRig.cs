...
// The original HoloToolkit.Unity.UX.BoundingBoxRig was modified in order to create a more
// intuitive rotation mechanic. When rotating the original bounding box rig, the AxisToAffect
// remained the same for each Gizmo handle. This resulted in counter-intuitive rotations after
// rotating a few times. The modified bounding box utilises similar logic as the original,
// except that the rig reverts to its original position on release, so it intuitively
// reflects the axis that the handles affect.
public class ModifiedBoundingBoxRig : MonoBehaviour, IInputClickHandler, IManipulationHandler
{
    ...
    private ModifiedBoundingBox.FlattenModeEnum flattenedAxis;
    ...
    private ModifiedBoundingBox boundingBoxPrefab;

    private ModifiedBoundingBox boxInstance;
    ...
    private ModifiedBoundingBoxGizmoHandle[] rigScaleGizmoHandles;

    private ModifiedBoundingBoxGizmoHandle[] rigRotateGizmoHandles;
    ...
    private int consecutiveInputClicks = 0;

    private Vector3 manipulationOriginalPosition;

    private float inputToObjectMovementScale = 10;

    public ModifiedBoundingBox BoundingBoxPrefab
    {
        get
        {
    ...
    private void Start()
    {
        ...
        boxInstance = Instantiate(BoundingBoxPrefab) as ModifiedBoundingBox;
    ...
    private void Update()
    {
        if (destroying == false && ShowRig && IsMoving)
        {
            UpdateBoundsPoints();
            UpdateHandles();
        }
    ...
    private void UpdateCornerHandles()
    {
        ...
        if (cornerHandles == null)
        {
            ...
            rigScaleGizmoHandles = new ModifiedBoundingBoxGizmoHandle[handleCentroids.Count];
            for (int i = 0; i < cornerHandles.Length; ++i)
            {
                ...
                rigScaleGizmoHandles[i] = cornerHandles[i].AddComponent<ModifiedBoundingBoxGizmoHandle>();
    ...
    private void UpdateRotateHandles()
    {
        ...
        if (rotateHandles == null)
        {
            ...
            rigRotateGizmoHandles = new ModifiedBoundingBoxGizmoHandle[12];
            for (int i = 0; i < rotateHandles.Length; ++i)
            {
                ...
                rigRotateGizmoHandles[i] = rotateHandles[i].AddComponent<ModifiedBoundingBoxGizmoHandle>();
    ...
    {
        if (consecutiveInputClicks == 0) {
            StartCoroutine(ResetTapsAfterInterval(1.0f));
        } else if (consecutiveInputClicks == 1 && !ShowRig) {
            Activate();
            CreateHandles();
        }

        consecutiveInputClicks++;
    }

    IEnumerator ResetTapsAfterInterval(float interval) {
        yield return new WaitForSeconds(interval);
        consecutiveInputClicks = 0;
    }
    ...
    {
        transform.position = manipulationOriginalPosition + eventData.CumulativeDelta * inputToObjectMovementScale;
    }
